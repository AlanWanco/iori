...
impl EplusClient {
    ...
    pub async fn login(
        ...
        log::info!("Fetching Eplus authentication state...");
        let res = client
            .get(eplus_url)
            .send()
            .await
            .map_err(|_| anyhow::anyhow!("Failed to fetch Eplus authentication state."))?;
        ...
        let api_res = client
            ...
            .send()
            .await
            .map_err(|_| anyhow::anyhow!("Failed to send Eplus pre-login request."))?;
        api_res
            .error_for_status_ref()
            .map_err(|_| anyhow::anyhow!("Eplus pre-login request returned an error status."))?;

        let ft_auth: FtAuthResponse = api_res
            .json()
            .await
            .map_err(|_| anyhow::anyhow!("Failed to parse Eplus pre-login response."))?;
        if !ft_auth.is_success {
            bail!("Eplus pre-login authentication failed.");
        }
        ...
        let form_res = client
            ...
            .send()
            .await
            .map_err(|_| anyhow::anyhow!("Failed to submit Eplus login form."))?;
        form_res
            .error_for_status_ref()
            .map_err(|_| anyhow::anyhow!("Eplus login form returned an error status."))?;

        let final_login_url = form_res.url().to_string();
        if final_login_url.contains("member/login") {
            bail!("Eplus login failed: redirected back to the login page.");
        }

        log::info!("Eplus login successful.");
        Ok(Self { client })
    }
    ...
    pub async fn get_event_data(&self, eplus_url: &str) -> anyhow::Result<EplusEventData> {
        log::info!("Fetching Eplus event data...");
        let res = self
            .client
            .get(eplus_url)
            .send()
            .await
            .map_err(|_| anyhow::anyhow!("Failed to fetch Eplus event data."))?;
        res.error_for_status_ref()
            .map_err(|_| anyhow::anyhow!("Eplus event page returned an error status."))?;
        ...
        let body = res
            .text()
            .await
            .map_err(|_| anyhow::anyhow!("Failed to read Eplus event data."))?;
    ...
    fn parse_event_page(
        ...
        let delivery_status = app_data
            .delivery_status
            .as_deref()
            .map(DeliveryStatus::from_api_value)
        ...
        match &delivery_status {
            ...
            DeliveryStatus::Unknown(_) => log::warn!("Unknown Eplus delivery status received."),
        }
        ...
        Ok(EplusEventData {
            ...
            title,
            delivery_status,
            archive_mode: app_data.archive_mode,
            is_drm,
            m3u8_urls,
            stream_session,
            session_update_url,
    ...
    pub async fn refresh_status_cookies(
        ...
        log::info!("Refreshing Eplus stream status...");
        let res = self
            .client
            .get(session_update_url)
            .send()
            .await
            .map_err(|_| anyhow::anyhow!("Failed to refresh Eplus stream status."))?;
        res.error_for_status_ref()
            .map_err(|_| anyhow::anyhow!("Eplus status refresh returned an error status."))?;
    ...
    pub fn candidate_playlists(m3u8_urls: &[String], prefer_archive: bool) -> Vec<String> {
        ...
        for url in m3u8_urls {
            ...
            match host {
                "stream.live.eplus.jp" => live_urls.push(url.clone()),
                "vod.live.eplus.jp" => vod_urls.push(url.clone()),
                _ => continue,
            }
    ...
    pub async fn select_best_playlist(
        ...
        self.select_playlist_candidates(Self::candidate_playlists(m3u8_urls, prefer_archive))
            .await
    }

    pub async fn select_best_playlist_for_kind(
        &self,
        m3u8_urls: &[String],
        prefer_archive: bool,
    ) -> Option<String> {
        let candidates = Self::candidate_playlists(m3u8_urls, prefer_archive)
            .into_iter()
            .filter(|url| Self::is_archive_url(url) == prefer_archive);
        self.select_playlist_candidates(candidates).await
    }

    fn is_archive_url(url: &str) -> bool {
        Url::parse(url)
            .ok()
            .and_then(|url| url.host_str().map(|host| host == "vod.live.eplus.jp"))
            .unwrap_or(false)
    }

    async fn select_playlist_candidates(
        &self,
        ...
        for playlist_url in candidates {
            match self.client.get(&playlist_url).send().await {
                Ok(resp) if resp.status().is_success() => match resp.bytes().await {
                    Ok(body) if iori_hls::parse_playlist_res(&body).is_ok() => {
                        return Some(playlist_url);
                    }
                    Ok(_) => log::info!("Playlist candidate returned non-HLS content."),
                    Err(_) => log::info!("Playlist candidate body could not be read."),
                },
                Ok(resp) => log::info!(
                    "Playlist candidate is not accessible yet (HTTP {}).",
                    resp.status()
                ),
                Err(_) => log::info!("Playlist candidate request failed."),
            }
        }
...
#[cfg(test)]
mod tests {
    use super::EplusClient;

    #[test]
    fn playlist_candidates_only_accept_eplus_hosts() {
        let urls = vec![
            "https://stream.live.eplus.jp/out/v1/0123456789abcdef0123456789abcdef/index.m3u8"
                .to_string(),
            "https://evil.example/stream.live.eplus.jp/out/v1/0123456789abcdef0123456789abcdef/index.m3u8"
                .to_string(),
            "https://vod.live.eplus.jp/out/v1/archive/index.m3u8".to_string(),
        ];

        let live_candidates = EplusClient::candidate_playlists(&urls, false);
        let archive_candidates = EplusClient::candidate_playlists(&urls, true);

        assert!(live_candidates.iter().all(|url| {
            matches!(
                reqwest::Url::parse(url).unwrap().host_str(),
                Some("stream.live.eplus.jp" | "vod.live.eplus.jp")
            )
        }));
        assert_eq!(
            live_candidates[0],
        ...
        assert_eq!(
            ...
            "https://vod.live.eplus.jp/out/v1/archive/index.m3u8"
        );
    }
}
