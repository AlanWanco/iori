...
pub enum IoriCache {
    ...
    StreamDirFile(stream_dir::StreamDirCacheSource),
    #[cfg(feature = "opendal")]
...
impl IoriCache {
    ...
    pub fn stream_dir_file(path: impl Into<PathBuf>) -> IoriResult<Self> {
        Ok(Self::StreamDirFile(stream_dir::StreamDirCacheSource::new(
            path.into(),
        )?))
    }
...
impl CacheSource for IoriCache {
    async fn open_writer(&self, segment: &SegmentInfo) -> IoriResult<Option<CacheSourceWriter>> {
        match self {
            ...
            IoriCache::StreamDirFile(cache) => cache.open_writer(segment).await,
            #[cfg(feature = "opendal")]
    ...
    async fn open_reader(&self, segment: &SegmentInfo) -> IoriResult<CacheSourceReader> {
        match self {
            ...
            IoriCache::StreamDirFile(cache) => cache.open_reader(segment).await,
            #[cfg(feature = "opendal")]
    ...
    async fn segment_path(&self, segment: &SegmentInfo) -> Option<std::path::PathBuf> {
        match self {
            ...
            IoriCache::StreamDirFile(cache) => cache.segment_path(segment).await,
            #[cfg(feature = "opendal")]
    ...
    async fn invalidate(&self, segment: &SegmentInfo) -> IoriResult<()> {
        match self {
            ...
            IoriCache::StreamDirFile(cache) => cache.invalidate(segment).await,
            #[cfg(feature = "opendal")]
    ...
    async fn clear(&self) -> IoriResult<()> {
        match self {
            ...
            IoriCache::StreamDirFile(cache) => cache.clear().await,
            #[cfg(feature = "opendal")]
    ...
    fn location_hint(&self) -> Option<String> {
        match self {
            ...
            IoriCache::StreamDirFile(cache) => cache.location_hint(),
            #[cfg(feature = "opendal")]
