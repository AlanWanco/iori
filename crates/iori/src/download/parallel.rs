    ...
    C: CacheSource + Send + Sync + 'static,
{
    ...
    loop {
        ...
        let mut writer = match writer {
            Ok(writer) => writer,
            Err(error) => {
                tracing::warn!("Failed to open writer for {filename}: {error}. Retrying later.");
                retries -= 1;
                continue;
            }
        };
        ...
        let result = match result {
            Ok(()) => writer.shutdown().await.map_err(IoriError::IOError),
            Err(error) => Err(error),
        };
        drop(writer);
        ...
        match result {
            ...
            Err(error) => {
                ...
                tracing::warn!("Processing {filename} failed, retry later. {error}");
                retries -= 1;
            }
        }
    }
        ...
        while let Some(segments) = stream.next().await {
            ...
            for group in synchronized_groups {
                ...
                tokio::spawn(async move {
                    ...
                    let group_failed =
                        synchronized && outcomes.iter().any(|outcome| !outcome.succeeded);
                    ...
                    for outcome in outcomes {
                        ...
                        if group_failed || !outcome.succeeded {
                            ...
                            if let Err(error) = merger
                                .lock()
                                .await
                                .fail(outcome.segment, cache.clone())
                                .await
                            {
                                tracing::error!("Failed to mark {filename} as failed: {error}");
                            }
                        } else {
                            ...
                            if let Err(error) = merger
                                ...
                                .await
                            {
                                tracing::error!("Failed to mark {filename} as downloaded: {error}");
                            }
                        }
                    }

                    // Keep the permit until merger work for the entire group is complete.
                    drop(permit);
                });
            }
