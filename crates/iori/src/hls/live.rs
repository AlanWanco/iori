...
impl HlsLiveSource {
    ...
    /// subsequent fetches continue from there as normal. Zero is treated as unset.
    pub fn with_initial_segment_limit(mut self, limit: Option<usize>) -> Self {
        self.initial_segment_limit = limit.filter(|limit| *limit > 0);
        self
    }
}
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if is_first_fetch {
                    ...
                    if !is_end && let Some(limit) = initial_segment_limit {
